                ...
                if "missense_variant" in line[6]:
                    ...
                    if VALID and "*" in ensp_seq:
                        ...
                        if len(ensp_seq) == 0:
                            ...
                            print("Error: Reference protein sequence for {} ({}) contains '*', and no segment starts with 'M'. SKIPPING".format(enst, hugo))
                            VALID = False
                    ...
                    if VALID and orfPos != None:
                        ...
                        refSeq = enst_seq[orfPos-1:stopPos-1]

                        noStartStart = False
                            
                        if not refSeq.startswith("ATG"):
                            print('Error: Reference sequence for {} ({}) starting at position {} (based on reference protein matching ORF) does not start with a start codon.'.format(enst, hugo, orfPos))
                            noStartStart = True
                            print("SKIPPING")
                        ...
                        ## Move this outside the else if non-Start codon starts are fine to include.
                            ...
                            if ref != transRefNuc:
                                ...
                                ## refSeq is exactly the coding sequence, minus stop codon at end. If there are any stops, they should be interpreted as "U"
                                refProtein = "".join(Seq(refSeq, generic_rna).translate(to_stop=False)).replace("*","U")
                                ...
                                if mutCDNA.startswith("ATG"):
                                    ...
                                    ## translate mutant
                                    mutProtein = "".join(Seq(mutCDNA, generic_rna).translate(to_stop=False)).replace("*","U")
                                    ...
                                    if "del" in hgvs:
                                        ...
                                        if indelStartPos%3 == 1:
                                            ...
                                            if firstIndelCodon < 1:
                                                ...
                                                mutProtein = "".join(Seq(mutSeq, generic_rna).translate(to_stop=False)).replace("*","U")

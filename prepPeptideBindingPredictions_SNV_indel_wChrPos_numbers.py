...
Edited January 10, 2018:
    - Set up scripts for running on numbers cluster.
'''
...
NETMHCPAN_BIN = "/projects/sbrown_prj/bin/netMHCpan-4.0/netMHCpan"  ## v4 on projects
...
if __name__ == "__main__":
                            ...
                            if ref != transRefNuc:
                                ...
                                if mutCDNA.startswith("ATG"):
                                    ...
                                    if len(refPeptide) < MIN_PEPTIDE_LEN or len(mutPeptide) < MIN_PEPTIDE_LEN:
                                        ...
                                        pepsInfo[tumour][PEP_NUM] = "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(enst, "{}-{}-{}".format(mut_type,hgvs,aaChange), varPos, "mut", mutPeptide, refProtein, chrom, cpos)
                        ...
                        if VALID and orfPos != None:
                            ...
                            if not ensp_seq.startswith("M"):
                                ...
                                if mutPeptide:
                                    ...
                                    pepsInfo[tumour][PEP_NUM] = "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(enst, "{}-{}-{}".format(mut_type,hgvs,aaChange), pepVarPos, "mut", mutPeptide, refProtein, chrom, cpos)
    ...
    for tum in peps:
        ...
        ## write script file
        out = open(os.path.join(args.workingDir, tum, "script.sh"), "w")
        ## write header
        out.write("#!/bin/sh\n#SBATCH -o '{}'\n#SBATCH -e '{}'\n#SBATCH --mem=1000\n".format(os.path.join(args.workingDir, tum, "numbers.o"), os.path.join(args.workingDir, tum, "numbers.e")))
        out.write("source /projects/sbrown_prj/bin/pythonvenv/python3/bin/activate;cd {};{} -BA -tdir tmpdirXXXXXX -a {} -f peptides.fa > bindingRes.pMHC;\n".format(os.path.join(args.workingDir, tum), NETMHCPAN_BIN, hlas))
        out.close()

        #transferfof.write("{}\n".format(os.path.join(args.workingDir, tum, "peptides.fa")))
        #analysisfile.write("source /projects/sbrown_prj/bin/pythonvenv/python3/bin/activate;cd {};{} -tdir tmpdirXXXXXX -a {} -f peptides.fa > bindingRes.pMHC;\n".format(os.path.join(args.workingDir, tum), NETMHCPAN_BIN, hlas))
        analysisfile.write("{}\n".format(os.path.join(args.workingDir, tum, "script.sh")))
    ...
    idsfile.write("tumour\tminPeptideID\tENST\tmutation\tvarPos\tref_mut\tpepSequence\trefProtSeq\tchr\tpos\n")
    for tum in pepsInfo:
